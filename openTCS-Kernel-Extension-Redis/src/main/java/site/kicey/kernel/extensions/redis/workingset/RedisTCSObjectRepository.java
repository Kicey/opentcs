package site.kicey.kernel.extensions.redis.workingset;
...
public class RedisTCSObjectRepository implements TCSObjectRepository {

  private static final Logger LOG = LoggerFactory.getLogger(TCSObjectRepository.class);

  public static final String RedisMapName = "TCSObjects";

  private final RedissonClient redissonClient;

  private final Map<Class<?>, Map<String, TCSObject<?>>> objects;

  @Inject
  public RedisTCSObjectRepository(RedissonClient redissonClient) {
    this.redissonClient = redissonClient;
    objects = this.redissonClient.getMap(RedisMapName);
  }
  ...
  public void addObject(@Nonnull TCSObject<?> newObject)
    ...
    Map<String, TCSObject<?>> objectsByName = objects.computeIfAbsent(newObject.getClass(),
        k -> this.redissonClient.getMap(newObject.getClass().toString()));
    objectsByName.put(newObject.getName(), newObject);

    Set<TCSObject<?>> gottenObjets = getObjects(newObject.getClass());
    TCSObject<?> gottenObject = gottenObjets.iterator().next();
    if(gottenObject.equals(newObject)){
      LOG.info("Object stored and gotten are same.");
    }
  }
